...
from nnfs.activations.activation_functions import (
    linear,
    relu,
    sigmoid,
    softmax,
...
optimizer_func_defaults = {
    "adam": {"learning_rate": 0.01, "beta1": 0.65, "beta2": 0.6},
    "delta_bar_delta": {"theta": 0.1, "mini_k": 0.01, "phi": 0.1},
    "gradient_descent": {"learning_rate": 0.5},
    "gradient_descent_momentum": {"learning_rate": 0.09, "beta": 0.9},
    "rms_prop": {"learning_rate": 0.001, "beta": 0.99},
    "rprop": {"delta_max": 50, "delta_min": 0, "eta_plus": 1.1, "eta_minus": 0.5},
    "genetic": {"number_of_parents": 4, "fitness_eval": "accuracy"},
}
...
def error_func_manager():
    ...
    error_dropdown = st.selectbox(
        "Error function", ([err[0] for err in getmembers(error_functions, isfunction)])
    )
...
def optimizer_manager(initializer):
    ...
    optimizers = [
        cls_name
        for cls_name, cls_obj in getmembers(gradient)
        if "__" not in cls_name and cls_name != "gradient_optimizer"
    ]
    optimizers.extend(
        [
            cls_name
            for cls_name, obj_type in getmembers(non_gradient)
            if "__" not in cls_name
        ]
    )
    optimizers_select = st.selectbox("Choose a optimizer function", (optimizers))

    optimizer_func = optimizer_func_select[optimizers_select]
    # Initializer optimizer params and the default optimizer params
    init_params = initializer_optimizer(
        optimizer_func, **optimizer_func_defaults[optimizers_select]
    )
...
def initializer_optimizer(obj, **kwargs):
    ...
    **kwargs : dict
    ...
    for i in name_params:
        ...
        init_params[i] = st.text_input(
            f"Optimizer parameter: {i}", kwargs[i] if i in kwargs.keys() else 0
        )
...
def early_stop_manager():
    ...
    if st.checkbox("Use early stopping"):
        ...
        training_params = EarlyStopping(
            alpha=alpha_val, pkt_threshold=pkt_threshold_val, k_epochs=k_epochs_val
        )
    else:
...
def initializer_manager():
    ...
    optimizer_selector = st.selectbox(
        "Choose an initializer", ("heuristic", "xavier", "he", "none")
    )
    if optimizer_selector == "heuristic":
        slider_vals = st.slider("Select a range of values", -1.0, 1.0, (-0.3, 0.3))
        optimizer_param = dict(
            name="heuristic", lower=slider_vals[0], upper=slider_vals[1]
        )
...
def input_layer_manager(layer_size, colum1, colum2):
    with colum1:
        layer_size = st.number_input("Input layer", value=layer_size, disabled=True)
...
def output_layer_manager(layer_size, layer_num, colum1, colum2):
    with colum1:
        layer_size = st.number_input("Output layer", value=layer_size, disabled=True)
    activation_function = activation_func_manager(layer_num, colum2)
    act_function = activation_func_select[activation_function]
    return (layer_size, act_function)
