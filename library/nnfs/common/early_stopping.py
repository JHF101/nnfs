...
class EarlyStopping:
    ...
    def __init__(self, k_epochs=None, pkt_threshold=None, alpha=None):
        if (k_epochs is None) or (pkt_threshold is None) or (alpha is None):
            raise NotImplementedError(
                "k_epochs, pkt_threshold or alpha was not correctly initialized"
            )
    ...
    def training_progress(self, epoch_error_training_plot, pkt_threshold):
        """A moving average calculation that determines how much the average
        training error in a strip n+1 to n+k is larger that the minimum
        training error in that strip.
        ...
        pkt_threshold : float
            Feedback variable to prevent training progress failure
            for values less than k
        """
