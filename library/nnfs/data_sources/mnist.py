        ...
        for file in files:
            if file.endswith("ubyte"):
                with open(self.DATASET_DIR + file, "rb") as f:
                    ...
                    if (magic_number == training_set_image_file) or (
                        magic_number == test_set_image_file
                    ):
                        data_type = "images"
                        ...
                        parsed_data = np.frombuffer(
                            data, dtype=np.uint8, offset=16
                        )  # read the pixel data
                        parsed_data = parsed_data.reshape(
                            length_of_arr, row_count, col_count
                        )

                    # Just for completeness
                    elif (magic_number == training_set_label_file) or (
                        magic_number == test_set_label_file
                    ):
                        data_type = "labels"
                        parsed_data = np.frombuffer(
                            data, dtype=np.uint8, offset=8
                        )  # read the label data
