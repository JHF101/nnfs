...
class Optimizer(Initializers):
    ...
    def init_network_structures(self):
        ...
        return NotImplementedError(
            "Please make sure the optimizer is initializing network"
        )
        ...
        if "activation_functions" in kwargs.keys():
            ...
            if layer_len == activation_func_len:
                ...
                for i in range(0, layer_len):
                    log.info(
                        f"Layer {i}'s activation function is: {self.activation_functions[i]}"
                    )
            else:
                raise Exception(
                    f"Lengths of activation functions are not the same ({activation_func_len}!={layer_len})."
                )
    ...
    def init_propagations(self):
        ...
        return NotImplementedError(
            "Please make sure the propagation methods are being used."
        )
    ...
    def forward_prop_fit(self):
        ...
        return NotImplementedError(
            "Please make sure the forward propagation is being used."
        )
