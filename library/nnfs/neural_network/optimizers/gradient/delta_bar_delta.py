...
class DeltaBarDelta(GradientOptimizer):
    ...
    def optimize(self, **kwargs):
        ...
        if self.dbd_init_process:
            log.warning("Initialized dbd process")
            self.learning_rates_weights = [
                np.random.uniform(low=0.05, high=0.2, size=dE_dwij_t[i].shape)
                for i in range(len(dE_dwij_t))
            ]
            self.learning_rates_weights_deltas = [
                np.random.uniform(low=0.05, high=0.2, size=dE_dwij_t[i].shape)
                for i in range(len(dE_dwij_t))
            ]
            self.average_bar_delta_weights = [
                np.zeros(dE_dwij_t[i].shape) for i in range(len(dE_dwij_t))
            ]

            if self.use_bias:
                self.learning_rates_bias = [
                    np.random.uniform(low=0.05, high=0.2, size=dE_dbij_t[i].shape)
                    for i in range(len(dE_dbij_t))
                ]
                self.learning_rates_bias_deltas = [
                    np.random.uniform(low=0.05, high=0.2, size=dE_dbij_t[i].shape)
                    for i in range(len(dE_dbij_t))
                ]
                self.average_bar_delta_bias = [
                    np.zeros(dE_dbij_t[i].shape) for i in range(len(dE_dbij_t))
                ]
        ...
        for dw in range(len(dE_dwij_t)):  # TODO: Check if this can be expanded to RPROP
            if len(mega_delta_weights_array) > 1:
                ...
                self.average_bar_delta_weights[dw] = (1 - self.theta) * dE_dwij_t[
                    dw
                ] + self.theta * mega_delta_weights_array[-2][
                    dw
                ]  # self.theta*self.average_bar_delta_weights[dw]# self.theta * self.mega_delta_weights_array[-2][dw] #self.theta*self.average_bar_delta_weights[dw] ##
            else:
            ...
            # Else condition
            self.learning_rates_weights_deltas[dw] = np.where(
                condition_param == 0, 0.0, self.learning_rates_weights_deltas[dw]
            )
            # Greater than condition
            self.learning_rates_weights_deltas[dw] = np.where(
                condition_param > 0, self.mini_k, self.learning_rates_weights_deltas[dw]
            )
            # Less that condition
            self.learning_rates_weights_deltas[dw] = np.where(
                condition_param < 0,
                -self.phi * self.learning_rates_weights[dw],
                self.learning_rates_weights_deltas[dw],
            )
        ...
        if self.use_bias:
            for db in range(len(dE_dbij_t)):
                ...
                if len(mega_delta_bias_array) > 1:
                    self.average_bar_delta_bias[db] = (1 - self.theta) * dE_dbij_t[
                        db
                    ] + self.theta * mega_delta_bias_array[-2][
                        db
                    ]  # self.theta*self.average_bar_delta_bias[db] #self.theta*self.mega_delta_bias_array[-1][db]
                else:
                ...
                # First condition
                self.learning_rates_bias_deltas[db] = np.where(
                    bias_condition_param == 0, 0.0, self.learning_rates_bias_deltas[db]
                )
                self.learning_rates_bias_deltas[db] = np.where(
                    bias_condition_param > 0,
                    self.mini_k,
                    self.learning_rates_bias_deltas[db],
                )
                self.learning_rates_bias_deltas[db] = np.where(
                    bias_condition_param < 0,
                    -self.phi * self.learning_rates_bias[db],
                    self.learning_rates_bias_deltas[db],
                )
