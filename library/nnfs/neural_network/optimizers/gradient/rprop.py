    ...
    def optimize(self, **kwargs):
        ...
        if self.rprop_init_process:
            ...
            for w in range(0, len(weights)):
                ...
                self.del_ij_t_w.append(
                    np.random.uniform(
                        low=0.05, high=0.2, size=gradient_multiplication.shape
                    )
                )
                self.dE_dwij_t_1.append(
                    np.random.uniform(
                        low=0.05, high=0.2, size=gradient_multiplication.shape
                    )
                )
                ...
                if self.use_bias:
                    self.del_ij_t_b.append(
                        np.random.uniform(
                            low=0.05, high=0.2, size=bias_multiplication.shape
                        )
                    )
                    self.dE_dbij_t_1.append(
                        np.random.uniform(
                            low=0.05, high=0.2, size=bias_multiplication.shape
                        )
                    )
        ...
        if self.use_bias:
            if len(dE_dwij_t) != len(dE_dbij_t):
                raise Exception(
                    "Length of bias gradients does not equal the length of the weights gradients"
                )

        for outer in range(
            0, len(dE_dwij_t)
        ):  # Looping through the first set of weights
            ...
            for p in range(0, dE_dwij_t[outer].shape[0]):
                ...
                for q in range(0, dE_dwij_t[outer].shape[1]):
                    ...
                    if gradient_mult > 0.0:  # This loop only runs once
                        self.del_ij_t_w[outer][p][q] = np.minimum(
                            self.del_ij_t_w[outer][p][q] * self.eta_plus, self.delta_max
                        )
                        del_w_ij_t = (
                            -1.0
                            * np.sign(dE_dwij_t[outer][p][q])
                            * self.del_ij_t_w[outer][p][q]
                        )
                        weights[outer][p][q] = weights[outer][p][q] + del_w_ij_t
                        self.dE_dwij_t_1[outer][p][q] = dE_dwij_t[outer][p][q]

                    elif gradient_mult < 0.0:
                        self.del_ij_t_w[outer][p][q] = np.maximum(
                            self.del_ij_t_w[outer][p][q] * self.eta_minus,
                            self.delta_min,
                        )  # Error was delta min was huge
                ...
                for s in range(0, dE_dbij_t[outer].shape[0]):
                    for r in range(0, dE_dbij_t[outer].shape[1]):
                        ...
                        if bias_grad_mult > 0.0:  # This loop only runs once
                            ...
                            self.del_ij_t_b[outer][s][r] = np.minimum(
                                self.del_ij_t_b[outer][s][r] * self.eta_plus,
                                self.delta_max,
                            )
                            del_b_ij_t = (
                                -1.0
                                * np.sign(dE_dbij_t[outer][s][r])
                                * self.del_ij_t_b[outer][s][r]
                            )
                            bias[outer][s][r] = bias[outer][s][r] + del_b_ij_t
                            self.dE_dbij_t_1[outer][s][r] = dE_dbij_t[outer][s][r]

                        elif bias_grad_mult < 0.0:
                            self.del_ij_t_b[outer][s][r] = np.maximum(
                                self.del_ij_t_b[outer][s][r] * self.eta_minus,
                                self.delta_min,
                            )  # Error was delta min was huge
